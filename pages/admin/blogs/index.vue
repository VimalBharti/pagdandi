<template>
    ...
    <div class="overflow-hidden dark:bg-slate-900 dark:border-gray-700">
        <!-- Header -->
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="sm:col-span-1">
                ...
                <div class="relative">
                    <input type="text" id="hs-as-table-product-review-search" name="hs-as-table-product-review-search" class="focus:outline-none py-2 px-3 pl-11 block w-full border-gray-200 shadow-sm rounded-md text-sm border" placeholder="Search">
            ...
            <!-- End Input -->
            <NuxtLink to="/admin/blogs/add" class="text-sm font-bold">+ Add New</NuxtLink>
            ...
            <template #default>
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <tr class="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800" v-for="blog in blogs" :key="blog.id">
                            <td class="h-px w-px whitespace-nowrap">
                                <NuxtLink :to="`/blog/${blog.slug}`" class="block h-full p-6">
                                    <div class="flex items-center">
                                        <span class="block font-semibold text-gray-800 dark:text-gray-200">{{ blog.title }}</span>
                                    </div>
                                </NuxtLink>
                            </td>
                            <td class="h-px w-72 min-w-[18rem]">
                                <a class="block h-full p-6" href="#">
                                    <span class="block text-sm text-gray-500">{{ blog.meta_seo }}</span>
                                </a>
                            </td>
                            <td class="h-px w-px whitespace-nowrap">
                                <a class="block h-full p-6" href="#">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">{{ $dayjs(blog.created_at).format('D MMM YYYY') }}</span>
                                </a>
                            </td>
                            <td class="h-px w-px whitespace-nowrap">
        ...
        <!-- Footer -->
        <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t">
            <div class="max-w-sm space-y-3">
                <select class="py-2 px-3 block w-full border-gray-200 rounded-md text-sm border shadow-inner focus:outline-none" v-model="limit" @change="fetchData()">
